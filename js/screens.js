...
function topbarTransparent() {
  ...
  <div class="topbar">
    <div class="topbar-left">
      ...
      ${STATE.notifications > 0 ? `<button class="icon-btn" data-toast="3 new notifications"><span class="notif-dot"></span>${ICONS.bell}</button>` : ''}
      <button class="icon-btn" data-go="#/shop">${ICONS.shop}</button>
    </div>
    <div class="topbar-right">
      <div class="pill" data-do="cycle-community-filter">${communityFilterLabel()} ${ICONS.chevronDown}</div>
    </div>
  </div>`;
}

function communityFilterLabel() {
  return ({all:'All', pins:'Pins', posts:'Posts', havens:'Havens'})[STATE.communityFilter] || 'All';
}
...
function screenCommunity() {
  ...
  const pins = MAP_PINS.filter(p => f === 'havens' ? p.type === 'haven' : true);
  return `
  ...
  <div class="map-filterbar">
    ${[['havens','Pack Havens'],['pins','Resource Partners'],['posts','Map Posts'],['all','Show All']].map(([k,l]) => `<div class="pill ${f===k?'active':''}" data-do="set-community-filter" data-filter="${k}">${l}</div>`).join('')}
  </div>
